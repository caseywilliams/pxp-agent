...
Util::CommandObject Task::buildCommandObject(const ActionRequest& request)
{
    ...
    auto executable_path = fs::path { task_file };
    auto builtin = BUILTIN_TASK_INTERPRETERS.find(executable_path.extension().string());
    if (builtin != BUILTIN_TASK_INTERPRETERS.end()) {
        auto details = builtin->second(executable_path.string());
        task_command.executable = details.first;
        task_command.arguments = details.second;
    } else {
        task_command.executable = executable_path.string();
    }

    if (implementation.input_method == "powershell") {
        // Run any powershell code using the powershell shim
        task_command.executable = (exec_prefix_ / "PowershellShim.ps1").string();
        // The shim accepts the original name of the task file as an argument
        task_command.arguments.push_back(task_file.string());
        // Pass any input on stdin ($input)
        task_command.input = task_params.toString();
    } else {
        // Pass any input on stdin
